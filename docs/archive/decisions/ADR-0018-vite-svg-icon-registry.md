...
每个图标以独立文件放在 `apps/frontend/src/assets/icons/`。Vite 使用 `vite-plugin-svg-icons` 的 `createSvgIconsPlugin` 在构建期生成 SVG sprite；应用入口加载 `virtual:svg-icons-register`，共享图标注册表读取 `virtual:svg-icons-names` 并导出稳定名称及菜单下拉选项。
...
- 插件虚拟模块必须同时纳入 Vite 和 TypeScript 类型环境。
- `fast-glob` 作为前端显式依赖固定，避免插件在 pnpm 严格依赖环境中因未声明传递依赖而启动失败。
- 图标名称注册表保持生产环境的数组导出路径。

## 验证和复审条件

- 前端类型检查和生产构建必须通过。
- 维护者人工验收名称清单、已知图标渲染和未知图标回退。
