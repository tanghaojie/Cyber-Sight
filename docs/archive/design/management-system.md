...
updated: 2026-07-27
archived: 2026-07-28
replaced_by: docs/design/README.md
---
...
在现有 Vue 3、Fastify、共享运行时 Schema 和 PostgreSQL 脚手架上提供可直接扩展的管理后台基线，包括现代化应用框架、用户登录退出、用户/角色/菜单/字典管理，以及全表统一的审计与软删除能力。
...
继续使用现有 Fastify、Zod、Drizzle、PostgreSQL、Vue Router、Pinia 与共享 fetch Client。密码散列、随机令牌和摘要使用 Node.js 内置 `crypto`。前端视觉体系使用 Tailwind CSS、Element Plus、项目 CSS 令牌和布局组件，具体边界见[前端应用壳设计](frontend-shell.md)。

## 失败模式与安全考虑

- 登录失败统一返回业务错误码，不泄露账号是否存在；密码与会话明文不写日志、不入库。
- 未认证访问管理 API 返回 HTTP 401；前端路由守卫跳转登录页。
- 用户名、用户邮箱、角色编码、菜单编码和字典类型+值只在未删除记录中唯一，用户角色、
  角色菜单组合只在未删除关系中唯一；均由数据库部分唯一索引保证并发安全，软删除后可
  复用业务身份并保留多条历史记录。会话令牌哈希属于不可复用安全标识，继续保持全表唯一。
...
- 共享契约构建、运行时输入校验、类型检查、Vitest、生产构建和 Drizzle 迁移生成全部通过。
