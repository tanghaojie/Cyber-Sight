...
页面放在 `apps/frontend/src/platform/modules/about/`，因为它属于 Cyber-Sight 产品平台能力；Foundation 的应用壳只提供承载和注册机制。后续企业资料或具体业务介绍应放在 `apps/frontend/src/platform/modules/<module>/`，不能复用本模块承载无关业务内容。

## 公共接口与路由

- `registerViews.ts` 登记稳定组件键 `about`，供数据库菜单动态路由加载。
- 菜单迁移提供 `/about`、`component = about`、`layout = AdminLayout` 的根级菜单。
- 页面内部使用 `<a>` 打开 `usePlatformConfig().githubUrl`，不新增 API 或权限键。
