...
- Flight 插件默认关闭，用户开启后以独立 `CustomDataSource` 显示内置模拟航线；每条航线始终以起点、终点之间的完整大圆线展示，活动航空器由随前端构建发布的本地 SVG 飞机图标而非点位表示，位置仍由唯一 `viewer.clock` 的时间样本插值定位；关闭或页面销毁会清空图层且不会产生网络请求；
...
6. 模拟航班默认关闭；开启后每条航线完整连接其起点和终点，活动位置显示为明确标示的飞机图标，时间轴播放、暂停与拖动会驱动位置，关闭和页面离开后无残留实体或网络错误；
