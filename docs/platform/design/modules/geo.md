...
- Flight 插件默认关闭，用户开启后以独立 `CustomDataSource` 显示内置模拟航线；每条航线始终以起点、终点之间的完整大圆线展示，活动航空器由透明 Canvas 绘制的飞机轮廓而非点位表示，位置仍由唯一 `viewer.clock` 的时间样本插值定位；关闭或页面销毁会清空图层且不会产生网络请求；
