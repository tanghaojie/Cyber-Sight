...
2026-09-11 起，外部 glTF/GLB 按[统一模型渲染标准](geo-model-rendering.md)自动跟随同一 Clock：Scene 拥有模型渲染管理器并发布 `scene.modelRendering`，Data 显式依赖 Scene 并在加载、变换、移除时登记/更新/注销。模型 Emissive 作为夜景通道，按模型所在地太阳高度平滑启闭；环境光和直射光按模型调整，保留原生 PBR、Unlit 和透明度语义。用户只需外部 URL 与定位，不选择 Profile，不建设模型资产。现有地球光照开关明确标注为“地球光照”，仅控制地球表面；模型昼夜自动跟随时间，阴影仍由 Scene 全局开关控制。此标准不改变 3D Tiles 科技扫描或 Flight。
...
- 外部 glTF/GLB 加载期间，数据面板必须显示模型专属的 loading 状态并阻止重复提交；模型加入场景后只有在 `Model.ready === true` 或 `readyEvent` 发出后才可读取包围球、注册为已加载资源或执行自动/手动定位。
- 外部模型首次渲染前接入 Scene 的统一渲染管理器；URL 与 WGS84 经纬度/椭球高为主要输入，缩放/姿态为可选调整。实际 glTF 材质检查发现无发光通道或 Unlit 时显示能力说明，不猜测窗灯或生成资产；移除和失败时先注销渲染，再销毁 Primitive。
...
- 动态任务轨、上下文面板、右侧属性检查器、插件错误提示、网页定位、右上地图控制、可收起状态条以及初始化、失败和重试状态均按审定方向实现；任务轨在垂直空间不足时内部滚动并使用与 Geo 主题一致的窄滚动条，状态条收起后不渲染坐标、FPS 或活动工具信息，右上指南针显示真实相机 heading 并支持点击回正；
- Shell 已移除按视口宽度隐藏、重排、缩窄或裁剪内容的媒体查询，上下文面板、属性检查器和错误提示使用审定的桌面固定宽度；
- 数据插件提供多源影像目录、图层显示/排序/定位、GeoJSON、glTF/GLB 和 3D Tiles 会话加载；外部 glTF/GLB 使用 WGS84 经度、纬度、椭球高和本地 heading/pitch/roll 构造固定坐标框架，模型 URL 默认填写维护者确认的演示 glTF，加载期间在外部数据面板显示模型专属 loading 状态，加入场景后等待 Cesium Model ready，再自动飞到模型并可继续编辑变换或按真实包围球再次定位；启动只加载 Google · 混合底图，底图目录支持角色筛选、搜索、局部滚动和可恢复的瓦片降级状态；`activeTilesetCapability` 向模型插件发布当前 3D Tiles，而不是穿透导入插件内部实现；
...
- Data 插件发布贡献后异步加载维护者确认的成都建筑 3D Tiles 启动预置，成功后自动定位；预置使用不叠加 `Cesium3DTileStyle` 的程序化 `CustomShader` 生成深蓝楼体、青色窗格/轮廓、紫色高层点缀和扫描带，扫描相位只在唯一 `viewer.clock` 播放时推进且不创建额外渲染循环。材质自发光不创建人工场景光源或全屏 Bloom；资源卡片允许在科技扫描与原始材质之间切换，手工加载的第三方 Tileset 默认保持原始材质；
- 成都建筑预置记录用户显示意图，并在相机椭球高严格大于 30 km 时临时隐藏、回到阈值内时恢复该意图；Data 纯工具拥有 Shader、Clock/场景监听器和 Tileset 的释放责任，远程加载失败只影响 Data 局部状态；
- 高德候选源由影像适配层按默认 `auto` 策略执行 GCJ-02 到 WGS84 的瓦片坐标校正；当前图层同时展示坐标校正与降级状态；
- Geo Viewer 启动视角使用成都近景预置：经度 `104.0700°`、纬度 `30.5000°`、椭球高 `1,600 m`、heading `360.0°`、pitch `-20.0°`、roll `0°`；默认建筑异步加载后的自动定位也回到该预置，避免覆盖启动状态。视图和场景插件提供全球/中国/成都定位、相机参数、2D/3D/哥伦布模式、视距限制以及太阳、月亮、大气、光照、阴影、地球底色、深度检测等设置；视图 controller 订阅相机变化、移动结束和场景模式切换完成事件，让面板快照跟随真实 Viewer，并保证最小视距不高于最大视距。Cesium 场景形态过渡帧中的 heading、pitch 和 roll 可能暂不可用，快照与 Shell 指南针必须保留上一次有效朝向或使用稳定默认值，不能把未定义值传给角度转换；
- Time 插件通过 `bottomDocks` contribution 提供无界 UTC 时间轴：可见窗口可持续平移，单屏范围可在 1 分钟至 10 年缩放，刻度按秒、分、时、天、月、年自动适配；时间轴和状态条默认收起，用户仍可独立展开或收起，时间轴收起后持续显示播放状态、当前 UTC 时间与展开按钮的紧凑把手。播放/暂停、游标定位、回到当前时刻和 `1×` 至 `3600×` 倍速仍只操作唯一 `viewer.clock`。工作台 Shell 根据状态条和 dock 的开合状态调整所有底部占位：两者展开时共享连续底部工作台，状态条收起时 dock 下沉到底部并避让展开按钮，时间轴收起时相应释放侧栏和 Cesium credits 的垂直空间；Viewer 禁止 DataSource 自动接管 Clock，Scene 通过 capability 向时间轴提供默认开启的太阳光照与默认关闭的太阳阴影；
...
- 外部 glTF/GLB 下载、资源创建或 WebGL ready 失败：模型操作显示可诊断错误，清理已加入场景的 Primitive，不把未 ready 的模型注册为可定位资源；成功后自动飞行只读取 ready 模型的包围球。
...
3. 工具轨、上下文面板、属性检查器和底部工作台的层级、开合与焦点可理解；工具轨内容超高时可独立滚动，状态条和时间轴同时展开时共用一张连续表面且没有双重阴影或间隙，任一层或两层收起时各自展开按钮可见、可键盘操作且不互相遮挡；
4. 所有已实现任务组均可进入，互斥鼠标工具切换后没有残留事件或临时实体；
5. 影像、地形、模型和 3D Tiles 单项加载失败不影响其他能力；启动成都建筑成功后自动定位，科技扫描/原始材质切换不破坏模型工具，扫描只在共享时间播放时推进；相机高度大于 30 km 时预置隐藏，回到不高于阈值时按用户此前显示选择恢复；
