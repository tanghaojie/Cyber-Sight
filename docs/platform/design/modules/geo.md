...
- 源卡片同时展示坐标系、描述、当前状态和禁用原因；缺少公开令牌或未启用坐标校正时可以保持不可用，但不能只以灰色表现；
...
- 地形面板按“地形剖面”“等高线”“地形着色”“坐标淹没”分区；地形剖面通过地图画线取得路径，淹没分析继续接受多边形坐标，不与等高线共享前置条件；
...
- 采样失败、取消、清除或切换工具时必须释放事件处理器和临时预览实体；静态渲染模式下的交互与结果写入都显式请求重绘；
...
- 右上地图控制：相机复位、2D/3D、真实指南针和全屏等高频地图动作；指南针跟随相机 heading 旋转，点击只将当前视角回正到正北朝上，不改变位置或俯仰角；
- 底部轻量状态条：鼠标经纬度、高程、相机高度、FPS/加载状态和活动工具提示；允许用户手动收起，收起后只保留展开按钮，不显示任何状态信息。
...
- 右上地图控制保持紧凑，底部状态条统一承载坐标、相机、渲染状态和活动工具指引；状态条收起时，常驻底部时间轴自动下沉到工作台底部，并为展开按钮保留独立点击区域；
...
- 动态任务轨、上下文面板、右侧属性检查器、插件错误提示、右上地图控制、可收起状态条以及初始化、失败和重试状态均按审定方向实现；状态条收起后不渲染坐标、FPS 或活动工具信息，右上指南针显示真实相机 heading 并支持点击回正；
...
- Time 插件通过 `bottomDocks` contribution 提供 UTC 当日 24 小时循环时间轴，支持播放/暂停、拖动、回到当前时刻和 `1×` 至 `3600×` 倍速；工作台 Shell 根据状态条开合状态调整所有底部 dock 的占位，展开时位于状态条上方，收起时下沉到底部并避让展开按钮；Viewer 禁止 DataSource 自动接管 Clock，Scene 通过 capability 向时间轴提供默认开启的太阳光照与默认关闭的太阳阴影；
...
| 地形采样、淹没、等高线与地形着色               | 已迁移并按任务分区   | `terrain` 插件；画线生成地形剖面，等高线使用地形材质，坐标仅用于淹没  |
...
- 外部服务 CORS、限频、令牌或网络失败：只把对应影像图层标记为 `degraded` 并保留最近错误，真实瓦片请求恢复成功后清回 `ready`；Natural Earth II 本地兜底保持在远程默认底图下方，不销毁 Viewer，也不因单次瓦片错误永久判定服务失败；地形 provider 只有最新请求成功安装后才提交为当前场景表面，失败或迟到结果保留当前 provider 和地形快照；
