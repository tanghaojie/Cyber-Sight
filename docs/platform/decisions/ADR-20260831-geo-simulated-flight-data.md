...
- 所有位置样本绑定已有的 `viewer.clock`，使用 `SampledPositionProperty`、`VelocityOrientationProperty` 和路径图形展示，不创建第二个 Clock，也不手动在每个 tick 写入位置；每条航线另以始末点之间的完整大圆线稳定呈现，活动位置使用内置飞机图标而非点位；
