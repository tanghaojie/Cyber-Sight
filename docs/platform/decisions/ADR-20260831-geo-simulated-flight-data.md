...
- 所有位置样本绑定已有的 `viewer.clock`，使用 `SampledPositionProperty`、`VelocityOrientationProperty` 和完整航线图形展示，不创建第二个 Clock，也不手动在每个 tick 写入位置；每条航线只以始末点之间的完整大圆线呈现，活动位置使用透明 Canvas 绘制、按屏幕前进方向旋转的飞机轮廓而非点位；
