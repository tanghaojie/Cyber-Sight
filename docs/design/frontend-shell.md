...
- `src/router/index.ts`：静态登录、404、应用壳，以及认证后动态子路由组装。
...
4. 运行时只为有效 `menu` 节点注册路由；`directory` 只分组，`button` 只打开 HTTP(S) 外链。
5. 菜单刷新或 401 清会话时移除动态路由并清空导航状态。

未知或空组件标识不生成路由；无效旧菜单仍可在管理页面修正，但不能进入执行导航。首次直接访问动态 URL 时，启动兜底路由在菜单加载后重新匹配，仍未知才进入 404。
...
- 注册表测试覆盖自动发现、重复/空标识和未知组件。
- 路由测试覆盖直接地址、导航刷新、404 和认证清理。
