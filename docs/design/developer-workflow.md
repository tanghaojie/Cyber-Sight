...
源码应让未阅读设计文档的开发者也能沿着模块入口理解主要职责、数据流和业务约束。注释正文统一使用中文，代码标识符、协议字段、库名称和无法准确翻译的术语保留原文。注释解释代码无法直接表达的意图和原因，不逐行翻译语法。

以下位置必须提供与复杂度相称的中文注释：

- 应用组合根、模块公共文件和框架注册入口，说明它们组装或公开了什么能力；
- HTTP Schema、路由、应用服务、仓储、Store 和公共组合式函数，说明边界职责及关键输入输出；
- 鉴权、密码、令牌、数据范围、缓存、事务、软删除、审计字段和错误映射，说明安全或一致性约束；
- 树转换、动态路由、递归、回退、去重、分页和多阶段状态同步，说明顺序、前置条件与特殊分支；
- Vue 单文件组件中不易从模板直接看出的页面分区、弹窗状态和提交链路。

命名和类型已经完整表达含义的导入、常量、简单赋值、事件转发和样板代码不强制注释。禁止为了提高注释数量而复述函数名或每行代码，也禁止保留与实现不一致的历史说明。修改受注释约束的实现时必须同步更新邻近注释；评审时把过期或误导性注释视为代码缺陷。
