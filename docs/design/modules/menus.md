...
目录可包含子目录、菜单或按钮；目录和菜单都必须配置站内路径，目录路径作为后代路由前缀但目录本身不可点击，菜单另配置组件标识，按钮配置 `http/https` 外链且布局必须为空。根节点路径必须以 `/` 开头；非根节点可使用相对路径，也可用以 `/` 开头的绝对路径覆盖上级前缀。空布局表示由前端继承最近目录布局，并最终回退默认布局。写入拒绝自引用、根节点相对路径和类型字段不匹配；树构造防止循环，按排序稳定输出。测试覆盖树构造、分层路径解析、布局/组件/外链校验、CRUD 与动态导航响应。
