...
目录可包含子目录、菜单或按钮；目录和菜单都必须配置站内路径，目录路径作为后代路由前缀但目录本身不可点击，菜单另配置组件标识，按钮配置 `http/https` 外链且布局必须为空。根节点路径必须以 `/` 开头；非根节点可使用相对路径，也可用以 `/` 开头的绝对路径覆盖上级前缀。空布局表示由前端继承最近目录布局，并最终回退默认布局。写入拒绝自引用、根节点相对路径和类型字段不匹配；树构造防止循环，按排序稳定输出。后端自动化测试覆盖树构造、写入校验、CRUD 与动态导航响应；前端分层路径解析、布局/组件/外链行为和表单交互由维护者人工验收。

写入模型保持严格类型约束；读取摘要兼容旧版本的不规范字段组合，避免存量单条记录阻断整个管理列表。运行时导航会过滤缺少站内路径/组件标识的菜单和缺少 HTTP(S) 地址的外链按钮；存量空目录路径按透明前缀只读兼容，旧记录仍可在菜单管理中编辑修复。

菜单编码 `code` 暂时保留为后端兼容键，但不再属于前端可配置或展示的信息。前端编辑时原样保留既有编码，新建时生成不透明内部编码；待后端契约、角色授权引用和数据库迁移另行完成后再物理删除该字段。
