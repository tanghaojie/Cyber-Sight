...
- 每个业务能力的请求、响应和事件 Schema 必须放在 `src/modules/<module>/` 独立目录，并由该目录的 `index.ts` 暴露公共契约。同一能力在前端和后端使用相同模块名。
- 契约包根入口只显式聚合各模块公共入口和真正通用的包装类型，不得直接导出模块内部辅助 Schema。一个模块不得从另一个模块的内部文件拼装契约；确需组合时只能使用对方公开的稳定 Schema。
