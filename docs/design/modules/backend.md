...
`apps/backend` 是默认服务端实现，使用 Fastify + TypeScript 提供快速开发体验。Java 不是当前并行实现，只有现实需求出现时才建立跨语言迁移方案。
...
Java 引入不是简单代码生成，需要独立的设计、ADR、迁移计划和契约回归测试。立项后从 Fastify `/docs/json` 导出并审查 OpenAPI，建立版本化的跨语言边界；在此之前不手写维护第二份契约。
