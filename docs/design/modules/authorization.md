...
模块在代码和迁移中登记稳定权限键，例如 `users.manage`、`roles.manage`、`departments.manage`、`menus.manage`、`dictionaries.manage` 和 `api_logs.read`。权限键至少包含两个以 `.` 分隔的片段，每个片段以小写字母开头，后续可使用小写字母、数字、连字符或下划线。`sys_permissions.key` 全局唯一且不复用；角色通过 `sys_role_permissions` 获得权限。
