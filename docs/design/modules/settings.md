...
桌面宽度下，保存的 `navigationMenuStyle` 直接决定导航呈现：`sidebar` 为固定侧栏，`top` 为顶部导航；两种布局的 Header 均显示侧栏开关。`max-width: 1023px` 的窄屏始终优先使用抽屉式侧边导航，且不改写用户保存的首选项；窗口恢复到桌面宽度后自动恢复保存的风格。抽屉通过 Header 开关打开，通过侧栏关闭按钮或遮罩关闭。
...
3. 保存侧边/顶部风格后刷新页面，桌面端恢复对应布局；窄屏强制抽屉侧边导航，恢复宽度后回到保存风格。
4. 任意宽度下 Header 均显示侧栏开关；抽屉可通过关闭按钮和遮罩关闭。
