...
持久化键为带版本号和数字用户 ID 的 `jtlab_tag_view_history:v1:<userId>`。Store 激活账号时解析并校验 JSON，拒绝非绝对路径、空标题和重复路径；后续每次变更同步写回。浏览器不提供 `localStorage`、JSON 损坏或写入失败时，模块降级为当前会话内存状态。
...
这是新的版本化存储键，没有旧数据迁移。未来数据结构变化时递增键版本，并明确迁移或丢弃策略。
