...
当前 `0000` 初始迁移和既有迁移不可改写。已追加 `0004_positions_management.sql`，创建两张 `sys_` 表、外键、索引、`positions.manage` 权限和岗位管理菜单，并为既有超级管理员角色补授权限；未写入示例岗位数据。
...
- Schema/迁移静态断言覆盖同部门名称唯一性、岗位部门字段、用户关系不重复保存部门 ID、追加迁移和权限种子；运行时代码实现岗位部门校验、岗位移动有任职时拒绝、软删除和历史关系恢复。
...
- 数据库：所有新表使用 `sys_` 前缀、审计字段、外键、部分唯一索引、`0004` 追加迁移和超级管理员权限种子；`schema.test.ts` 与 `migration.test.ts` 已增加岗位表、索引和迁移断言。
