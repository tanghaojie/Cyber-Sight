    ...
    >
      <el-form label-position="top" @submit.prevent="submit">
        <div class="form-columns">
          ...
          /></el-form-item>
          <el-form-item :label="editingId ? '新密码（可选）' : '密码'" :required="!editingId"
            ><el-input
        ...
        <div class="dialog-actions">
          ...
          ><el-button native-type="submit" type="primary" :loading="saving">保存用户</el-button>
        </div>
      </el-form>
    </el-dialog>
  </section>
...
function roleName(id: number): string {
  return roleOptions.value.find((role) => role.id === id)?.name ?? `角色 #${id}`
}
...
async function submit(): Promise<void> {
  ...
  try {
    if (
      !form.displayName ||
      !form.email ||
      (!editingId.value && (!form.username || form.password.length < 8))
    )
...
async function remove(row: UserSummary): Promise<void> {
  try {
    await ElMessageBox.confirm(`确定删除用户“${row.displayName}”吗？`, '删除用户', {
      type: 'warning',
