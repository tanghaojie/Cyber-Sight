...
describe('global HTTP error interceptor', () => {
  it.each([401, 404, 500])('passes HTTP %s to the installed application handler', async (status) => {
    const handler = vi.fn()
  ...
  it.each([200, 400, 403, 409, 429, 502])('leaves HTTP %s to the calling business module', async (status) => {
    const handler = vi.fn()
    const uninstall = installGlobalHttpErrorHandler(handler)
