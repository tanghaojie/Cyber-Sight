...
const foundationViewModules = import.meta.glob<ViewRegistrationModule>(
  // eager 让注册错误在应用启动阶段暴露，而不是首次访问菜单时才失败。
  '@/foundation/modules/**/registerViews.ts',
  { eager: true },
)

const registeredViews: Record<string, { label: ViewLabel; component: RouteComponent }> = {
  ...createViewRegistry(foundationViewModules),
}

export function registerViewModules(modules: Readonly<Record<string, unknown>>): void {
  const platformViews = createViewRegistry(modules)
  for (const [key, view] of Object.entries(platformViews)) {
    if (Object.hasOwn(registeredViews, key)) {
      throw new Error(`${DUPLICATE_VIEW_KEY} "${key}" registered by Platform`)
    }
    registeredViews[key] = view
  }
}

export const viewRegistry: Readonly<
  Record<string, { label: ViewLabel; component: RouteComponent }>
> = registeredViews
