...
try {
  ...
  const [schema] = await databaseClient<
    Array<{
      ...
      foundationMigrationsTable: string | null
      platformMigrationsTable: string | null
      uuidV7PrimaryKeyCount: number
    }>
  >`
    select
      to_regclass('public.sys_users')::text as "systemUsersTable",
      to_regclass('drizzle.__foundation_migrations')::text as "foundationMigrationsTable",
      to_regclass('drizzle.__platform_migrations')::text as "platformMigrationsTable",
      (
  ...
  console.log(
    [
      ...
      `foundationMigrationsTable=${schema.foundationMigrationsTable ?? 'missing'}`,
      `platformMigrationsTable=${schema.platformMigrationsTable ?? 'missing'}`,
