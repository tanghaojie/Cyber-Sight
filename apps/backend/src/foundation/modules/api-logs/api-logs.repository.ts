...
import { apiRequestLogs } from '@/foundation/database/schema.js'
import { DATABASE } from '@/foundation/shared/database/database.provider.js'
import { pageOffset } from '@/foundation/shared/database/pagination.js'
...
export class ApiLogsRepository {
  ...
  async insertApiLogEvents(events: ApiLogEvent[]): Promise<void> {
    if (events.length === 0) {
      return
    }
   ...
   */
  async deleteExpiredApiLogs(batchSize: number): Promise<number> {
    return this.db.transaction(async function deleteWithLock(tx) {
  ...
  async listApiLogs(query: NormalizedApiLogQuery) {
    ...
    const [rows, [{ value: total }]] = await Promise.all([
      ...
      this.db.select({ value: count() }).from(apiRequestLogs).where(predicate),
    ])
    return {
      total,
