...
import { userDepartments, userRoles, users } from '@/foundation/database/schema.js'
import { DepartmentsAccess } from '@/foundation/modules/departments/departments.access.js'
import type { DataAccessPlan } from '@/foundation/modules/authorization/authorization.service.js'
import { PositionsAccess } from '@/foundation/modules/positions/positions.access.js'
import { PositionsService } from '@/foundation/modules/positions/positions.service.js'
...
import { hashPassword, verifyPassword } from '@/foundation/modules/auth/auth.security.js'
...
export class UsersRepository {
  constructor(
    ...
    @Inject(PositionsAccess)
    private readonly positions: PositionsAccess,
  ...
  async listUsers(query: RepositoryListQuery, access: DataAccessPlan) {
    ...
    const [{ value: total }] = await this.db.select({ value: count() }).from(users).where(predicate)
    ...
    const [roleAssignments, departmentAssignments, positionAssignments] = ids.length
      ? await Promise.all([
          ...
          this.positions.activePositionAssignments(enabledIds),
        ])
      : [[], [], []]
  ...
  async hasValidAssignments(
    ...
    const [validRoleIds, validDepartmentIds, validPositions] = await Promise.all([
      ...
      this.departments.enabledDepartmentIds(departmentIds),
      this.positions.positionAssignmentsAreValid(positionIds, departmentIds),
    ])
    return (
  ...
  async createUser(input: UserCreate, actorId: EntityId): Promise<EntityId> {
    ...
    return this.db.transaction(async (tx) => {
      ...
      await this.replaceUserRoles(tx, created.id, input.roleIds, actorId)
      await this.replaceUserDepartments(
        tx,
        created.id,
        input.departmentIds,
      ...
      await this.positionsService.replaceUserPositionsInTransaction(
        tx,
        created.id,
        input.positionIds,
        input.departmentIds,
  ...
  async updateUser(
    ...
    return this.db.transaction(async (tx) => {
      ...
      await this.replaceUserRoles(tx, id, input.roleIds, actorId)
      await this.replaceUserDepartments(
        tx,
        id,
        input.departmentIds,
  ...
  async softDeleteUser(id: EntityId, actorId: EntityId, access: DataAccessPlan): Promise<boolean> {
    return this.db.transaction(async (tx) => {
      ...
      await this.positionsService.softDeleteUserPositionsInTransaction(tx, id, actorId)
      return true
    })
  }
}
